class CSVComparison {
  ...
  compareFiles() {
    ...
    // También mostrar toast para referencia rápida
  ...
  showComparisonSummaryPopup({ changed, added, deleted, unchanged }) {
    ...
    popup.innerHTML = `
      <div class="popup-overlay" style="background:transparent;backdrop-filter:none;">
        <div class="popup-content" style="background:#1a2332 !important; border-radius:20px; box-shadow:0 20px 60px rgba(0,0,0,0.45); border:1px solid rgba(255,255,255,0.10); max-width:700px; width:100%; color:#fff;">
          <div class="modal-header" style="background:#1a2332; border-radius:20px 20px 0 0; display:flex; align-items:center; justify-content:space-between; padding:1.2rem 2rem 1.2rem 2rem; border-bottom:1px solid rgba(255,255,255,0.10);">
            <div class="header-left" style="display:flex;align-items:center;gap:1rem;">
              ...
              <h3 class="panel-header-title" style="color:white;font-family:'Inter','Segoe UI',Arial,sans-serif;font-weight:600;font-size:1.5rem;margin:0;letter-spacing:0.02em;text-shadow:0 2px 8px rgba(0,0,0,0.3);">Comparison Results</h3>
            </div>
            <button class="close-btn" id="closeSummaryPopup" style="color:rgba(255,255,255,0.7);background:rgba(255,255,255,0.1);border:1px solid rgba(255,255,255,0.2);border-radius:50%;width:40px;height:40px;display:flex;align-items:center;justify-content:center;font-size:1.5rem;transition:all 0.3s ease;cursor:pointer;">×</button>
          </div>
          <div class="popup-body" style="padding:1.2rem 2rem 1rem 2rem;">
            <div class="summary-stats" style="text-align:center;margin-bottom:1.2rem;">
              <div class="stat-item total" style="background:rgba(71,178,229,0.1);border:1px solid rgba(71,178,229,0.3);border-radius:16px;padding:1.2rem;display:inline-block;">
                <div class="stat-number" style="font-size:2.2rem;font-weight:700;color:#47B2E5;margin-bottom:0.5rem;font-family:'Inter','Segoe UI',Arial,sans-serif;">${total}</div>
                <div class="stat-label" style="color:rgba(255,255,255,0.8);font-size:1.1rem;font-weight:500;">Total Records</div>
              </div>
            </div>
            <div class="summary-breakdown" style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;margin-bottom:1.2rem;">
              <div class="breakdown-item unchanged ${unchanged > 0 ? 'has-data' : ''}" style="background:rgba(16,185,129,0.1);border:1px solid rgba(16,185,129,0.3);border-radius:12px;padding:1rem;display:flex;align-items:center;gap:1rem;${unchanged > 0 ? '' : 'opacity:0.6;'}">
                <div class="breakdown-content" style="flex:1;">
                  <div class="breakdown-number" style="font-size:1.3rem;font-weight:700;color:white;margin-bottom:0.25rem;">${unchanged}</div>
                  <div class="breakdown-label" style="color:rgba(255,255,255,0.7);font-size:0.9rem;font-weight:500;">Unchanged</div>
                </div>
                <div class="breakdown-percentage" style="color:rgba(255,255,255,0.8);font-size:1rem;font-weight:600;flex-shrink:0;">${total > 0 ? Math.round((unchanged / total) * 100) : 0}%</div>
              </div>
              <div class="breakdown-item changed ${changed > 0 ? 'has-data' : ''}" style="background:rgba(251,191,36,0.1);border:1px solid rgba(251,191,36,0.3);border-radius:12px;padding:1rem;display:flex;align-items:center;gap:1rem;${changed > 0 ? '' : 'opacity:0.6;'}">
                <div class="breakdown-content" style="flex:1;">
                  <div class="breakdown-number" style="font-size:1.3rem;font-weight:700;color:white;margin-bottom:0.25rem;">${changed}</div>
                  <div class="breakdown-label" style="color:rgba(255,255,255,0.7);font-size:0.9rem;font-weight:500;">Modified</div>
                </div>
                <div class="breakdown-percentage" style="color:rgba(255,255,255,0.8);font-size:1rem;font-weight:600;flex-shrink:0;">${total > 0 ? Math.round((changed / total) * 100) : 0}%</div>
              </div>
              <div class="breakdown-item new ${added > 0 ? 'has-data' : ''}" style="background:rgba(59,130,246,0.1);border:1px solid rgba(59,130,246,0.3);border-radius:12px;padding:1rem;display:flex;align-items:center;gap:1rem;${added > 0 ? '' : 'opacity:0.6;'}">
                <div class="breakdown-content" style="flex:1;">
                  <div class="breakdown-number" style="font-size:1.3rem;font-weight:700;color:white;margin-bottom:0.25rem;">${added}</div>
                  <div class="breakdown-label" style="color:rgba(255,255,255,0.7);font-size:0.9rem;font-weight:500;">Added</div>
                </div>
                <div class="breakdown-percentage" style="color:rgba(255,255,255,0.8);font-size:1rem;font-weight:600;flex-shrink:0;">${total > 0 ? Math.round((added / total) * 100) : 0}%</div>
              </div>
              <div class="breakdown-item deleted ${deleted > 0 ? 'has-data' : ''}" style="background:rgba(239,68,68,0.1);border:1px solid rgba(239,68,68,0.3);border-radius:12px;padding:1rem;display:flex;align-items:center;gap:1rem;${deleted > 0 ? '' : 'opacity:0.6;'}">
                <div class="breakdown-content" style="flex:1;">
                  <div class="breakdown-number" style="font-size:1.3rem;font-weight:700;color:white;margin-bottom:0.25rem;">${deleted}</div>
                  <div class="breakdown-label" style="color:rgba(255,255,255,0.7);font-size:0.9rem;font-weight:500;">Removed</div>
                </div>
                <div class="breakdown-percentage" style="color:rgba(255,255,255,0.8);font-size:1rem;font-weight:600;flex-shrink:0;">${total > 0 ? Math.round((deleted / total) * 100) : 0}%</div>
              </div>
            </div>
            <div class="summary-message" style="text-align:center;background:rgba(71,178,229,0.1);border:1px solid rgba(71,178,229,0.2);border-radius:12px;padding:1rem;margin-top:1rem;">
              ${hasChanges 
                ? `<p style='color:white;margin:0;font-size:1.05rem;line-height:1.5;'><strong>${changed + added + deleted}</strong> changes detected across your data.</p>`
                : '<p style="color:white;margin:0;font-size:1.05rem;line-height:1.5;">No changes found. Both datasets are identical.</p>'
              }
            </div>
          </div>
          <div class="modal-footer" style="padding:1rem 2rem 1.2rem 2rem;display:flex;gap:1rem;justify-content:flex-end;border-top:1px solid rgba(255,255,255,0.1);background:rgba(255,255,255,0.02);">
            ${hasChanges 
              ? `<button class="modal-btn primary" id="viewDetailedResults">View Detailed Results</button>
                 <button class="modal-btn secondary" id="sendToMainTable">Send to Main Table</button>`
              : ''
            }
            <button class="modal-btn secondary" id="closeSummaryPopupBtn">Close</button>
          </div>
        </div>
      </div>
    `;
    ...
    const closeBtnFooter = popup.querySelector('#closeSummaryPopupBtn');
    const viewResultsBtn = popup.querySelector('#viewDetailedResults');
  ...
  resetAll() {
    ...
    this.compareColumns = [];
    this.comparisonResults = null;
    this.keyColumnsSelect.value = '';
    this.compareColumnsSelect.value = '';
    this.resultsTable.innerHTML = '';
    this.originalCSVInput.value = '';
    this.newCSVInput.value = '';
    if (this.originalVersionSelect) this.originalVersionSelect.value = '';
    if (this.newVersionSelect) this.newVersionSelect.value = '';
    this.startComparisonBtn.disabled = true;
    this.unchangedCount.textContent = '0';
    this.changedCount.textContent = '0';
    this.newCount.textContent = '0';
    this.deletedCount.textContent = '0';
    this.hideResultsModal();
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  console.log('CSV Comparison: DOM Content Loaded');
  const compareCSVBtn = document.getElementById('compareCSVBtn');
  console.log('Compare CSV button found:', compareCSVBtn);
  
  const csvComparison = new CSVComparison();
  console.log('CSV Comparison instance created:', csvComparison);

  compareCSVBtn.addEventListener('click', () => {
    console.log('Compare CSV button clicked');
...
function showComparisonToast({ changed, added, deleted, unchanged }) {
  ...
  toast.innerHTML = `
    ...
    <span style='margin-left:1.5em;cursor:pointer;font-size:1.3em;color:#e3f2fd;opacity:0.7;transition:opacity 0.2s;' title='Close'>&times;</span>
  `;
  const closeBtn = toast.querySelector('span[title="Close"]');
