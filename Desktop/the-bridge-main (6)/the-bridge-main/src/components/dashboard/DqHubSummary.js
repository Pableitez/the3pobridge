...
export class DqHubSummary {
      ...
      <div style="display: flex; align-items: center; justify-content: space-between; gap: 1rem;">
        <div style="display: flex; align-items: center; gap: 0.8rem;">
          <input type="checkbox" class="card-checkbox-input" ${isSelected ? 'checked' : ''} style="
            ...
            margin: 0;
          ">
          <h4 style="margin: 0; color: #1a2332; font-size: 1.1rem; font-weight: 600;">${cardData.name}</h4>
        </div>
        
        <div style="display: flex; align-items: center; gap: 0.5rem;">
          <span style="color: #47B2E5; font-weight: 600; font-size: 0.9rem;">${cardData.count.toLocaleString()} records</span>
        </div>
  ...
  _renderSummaryPreviewInternal() {
    ...
    if (!preview || this.isGeneratingPreview) {
      console.log('DQ Preview update skipped:', { preview: !!preview, isGenerating: this.isGeneratingPreview });
      return;
    }

    this.isGeneratingPreview = true;
    console.log('DQ Starting preview generation...');
    
    try {
      const selectedCardsData = Object.values(this.quickCardsData)
        .filter(card => this.selectedCards.has(card.id));

      console.log('DQ Rendering preview with', selectedCardsData.length, 'selected cards');
      
      // Log card configurations for debugging
      selectedCardsData.forEach(card => {
        const config = this.getCardConfig(card.id);
        console.log(`DQ Card "${card.name}" config:`, config);
      });
      ...
      requestAnimationFrame(() => {
        ...
        this.isGeneratingPreview = false;
        console.log('DQ Preview generation completed');
      });
    } catch (error) {
      console.error('DQ Error generating preview:', error);
  ...
  async copyToClipboard() {
    try {
      ...
      // Get the actual preview content with HTML formatting
      const previewElement = document.getElementById('dqSummaryPreview');
      if (!previewElement) {
        this.showNotification('Preview not available. Please wait for the preview to load.', 'warning');
        return;
      }

      // Get the HTML content to preserve table formatting
  ...
  convertToEmailFormat(html) {
    ...
    const styleReplacements = {
      ...
      'class="card-title"': 'style="font-weight: bold; color: #1a2332; margin-bottom: 10px;"',
      ...
      'class="table-header"': 'style="background: #1a2332; color: white; padding: 12px; text-align: left; border: 1px solid #ddd;"',
    ...
    emailHtml = `
      <div style="font-family: Arial, sans-serif; max-width: 800px; margin: 0 auto; padding: 20px;">
        <h2 style="color: #1a2332; border-bottom: 2px solid #47B2E5; padding-bottom: 10px;">
        ...
        <html>
          <head>
            ...
            <style>
              ...
              h1 { color: #1a2332; border-bottom: 2px solid #47B2E5; }
              .card-section { margin: 20px 0; padding: 15px; background: #f8f9fa; border-radius: 8px; }
              .card-title { font-weight: bold; color: #1a2332; }
  ...
  setupDuplicateColumnsSearch() {
    ...
    if (searchInput) {
      ...
      this.handleDuplicateColumnsSearch = (e) => {
        ...
        console.log(`✅ DQ Filtered ${checkboxes.length} checkboxes, ${visibleCount} visible`);
      };
